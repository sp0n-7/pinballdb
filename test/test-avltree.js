...
var bst = new AVLTree({ compareKeys: compareKeys });

// Inserting some data
bst.insert({ val: 15 }, 'some data for key 15');
bst.insert({ val: 12 }, 'something else');
bst.insert({ val: 18 }, 'hello');

// You can insert multiple pieces of data for the same key
// if your tree doesn't enforce a unique constraint
bst.insert({ val: 18 }, 'world');
...
console.log('between',bst.betweenBounds({ $lt: { val: 18 }, $gte: { val: 12 }}));   // Equal to ['something else', 'some data for key 15']

// Deleting all the data relating to a key
bst.delete({ val: 15 });   // bst.search(15) will now give []
bst.delete({ val: 18 }, 'world');   // bst.search(18) will now give ['hello'
