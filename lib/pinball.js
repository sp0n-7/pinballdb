  ...
  query(options) {
    ...
    if (!bOutsideBounds) {
      ...
      if (aLLIndexes && aURIndexes) {
        ...
        const fCheckActiveCount = (aArray) => {
          let nearbyMinutesThreshold = 20 * 60 * 1000 //20 minutes 
          let nActive = aArray.filter( (a) => { return (Date.now() - a.val) < nearbyMinutesThreshold }).length
          nActive = nActive < 100 ? nActive : 100
          if (nActive > N) {
            return nActive
          }
        ...
        if (nTotal > this.NBucketThreshold) {
          ...
          const sortAndSlice = (aData,NDim) => {
            return aData.sort( (a,b) => b.ts - a.ts).slice(0,NDim);
          }
            ...
            if (oMetaData) { // if non null
              ...
              if (matchingEvent(oData)) {
                ...
                if (aResults.length == NPlusPad) {
                  ...
                  N = fCheckActiveCount(aResults)
                  return sortAndSlice(aResults,N);
                }
              }              
            }
          }
