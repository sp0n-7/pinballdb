...
class Pinball {
  ...
  upsert(oDoc) {
        ...
        typeof oDoc.score     === 'number') 
    {
      ...
      const oPartialDoc = { id: oDoc.id, score: oDoc.score, latitude: oDoc.latitude, longitude: oDoc.longitude, ts: oDoc.ts, cs: oDoc.cs };
      // const oPartialDoc = { id: oDoc.id, score: oDoc.score, latitude: oDoc.latitude, longitude: oDoc.longitude, ts: oDoc.ts, cs: oDoc.cs };
  ...
  addSubscriber(options) {
    ...
    oProcs[`pb:${cityCode}:remove`] = (cacheId) => {
      const oDocId = cache.getDocId(cacheId);
    ...
    return cache.subscribeToCache(oSubscriberOptions).then( sub => {
      this.cacheSubscriber = sub; 

      const t0 = Date.now();
      const cacheDB = new Cache({ sCacheUrl: sCacheUrl });
      // const sKeyPattern = `pb:${cityCode}:*`;
      const setKey = cache.getSortedSetName(cityCode);
      // return cacheDB.batchImport({ setKey: setKey, fProc: this.upsert.bind(this), NBatch: 10000 }); // was always slower
      return cacheDB.orderedKeys({ setKey: setKey })
      .then( aKeys => {
        return cacheDB.batchGetFromCache(aKeys);
      })
      .then( aDocs => {
        console.log('aDocs.length',aDocs.length);
        this.batchUpsert(aDocs);
        return;
      })
    })
  }
  ...
  loadFromCache(options) {
    ...
    const setKey      = cache.getSortedSetName(cityCode);
    console.log({ action: sAction, setKey: setKey });

    return cacheDB.orderedKeys({ setKey: setKey })
    .then( aKeys => {
      t1 = Date.now();
      console.log({ action: sAction + '.scanned.keys',  length: aKeys.length, time: t1-t0 });
      return cacheDB.batchGetFromCache(aKeys);
    })
    .then( aObjects => {
      let t2 = Date.now();
      console.log({ action: sAction + '.batchGetFromCache', length: aObjects.length, time:t2-t1 });
      for (let i=0;i < aObjects.length;i++) {
        this.upsert(aObjects[i]);
      }
      let t3 = Date.now();
      console.log({ action: sAction + '.finishedUpsert', length: aObjects.length, time:t3-t2 });
    })
  }

  // returns Promise, async gets full json from cache
  queryFromCache(options) {
    const aPartialResults = this.query(options);
    const aCacheIds = aPartialResults.map( oResult => cache.getCacheId({ id: oResult.id, cityCode: this.cityCode }));
    return cacheDB.batchGetFromCache(aCacheIds);
  }
    ...
    if (!bOutsideBounds) {
      ...
      if (aLLIndexes && aURIndexes) {
        ...
        if (nTotal > this.NBucketThreshold) {
          ...
          // get N + 30 ordered by cs to review the last 30min-1hour or so
          // then sort and slice results to get top N 
          const NPlusPad = N + 30;    

          const sortAndSlice = (aData) => {
            return aData.sort( (a,b) => b.ts - a.ts).slice(0,N);
          }
