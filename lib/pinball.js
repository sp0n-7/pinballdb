...
class Pinball {
  constructor(options) {
    ...
    this.aOrdered = [];
    this.tLats    = new AVLTree({ compareKeys: compareKeys });
    this.tLons    = new AVLTree({ compareKeys: compareKeys });
  ...
  _removeFromTree(options) {
    ...
    if (oOldDoc) {
      tTree.delete({ val: val }, { id: id, val: val });
    }
  }
  ...
  query(options) {
    ...
    if (!bOutsideBounds) {
      ...
      let aLats = this.tLats.betweenBounds({ $lt: { val: lowerLatitude }, $gte: { val: upperLatitude } });       
      let aLons = this.tLons.betweenBounds({ $lt: { val: lowerLongitude}, $gte: { val: upperLongitude} });       

      console.log('aLats',aLats,'aLons',aLons,'aLatsDim',aLats.length,'aLonsDim',aLons.length,'fAreaRegion',fAreaRegion,'time betweens',Date.now()-t0);
