...
class Pinball {
  ...
  addSubscriber(options) {
    ...
    return cache.subscribeToCache(oSubscriberOptions).then( sub => {
      ...
      return this.loadFromCache({ sCacheUrl: sCacheUrl });
    })
  }
  ...
  loadFromCache(options) {
    ...
    this.cacheDB       = new Cache({ sCacheUrl : sCacheUrl });
    ...
    .then( aKeys => {
      ...
      return this.cacheDB.batchGetFromCache({ aCacheIds: aKeys, aProps: ['id','latitude','longitude','score','cs','ts']});
    })
    .then( aObjects => {
      let t2 = Date.now();
