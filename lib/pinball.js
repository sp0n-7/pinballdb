...
class Pinball {
  ...
  addSubscriber(options) {
    ...
    // // found another issue, after subscription is setup but before local pinball imports all data redis, need to queue new changes, then apply them to imported data
  ...
  loadFromCache(options) {
    ...
    .then( aKeys => {
      ...
      return this.cacheDB.batchGetFromCache({ aCacheIds: aKeys, aProps: ['id','latitude','longitude','score','cs','ts', 'll']});
    })
    .then( aObjects => {
      let t2 = Date.now();
