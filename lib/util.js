...
const binarySearch = (options) => {
  ...
  let val         = options.val;
  let sProp       = options && validString(options.sProp) ? options.sProp : 'val'
  ...
  while (minIndex <= maxIndex) {
    ...
    if (aArray[currentIndex] == null) {
      ...
      const currentVal = aArray[currentIndex][sProp];
  ...
  if (currentIndex >= 0 && aArray[currentIndex] && aArray[currentIndex][sProp] < val) {
    currentIndex++;
  }
...
// finds an insertion point for the given value
const binarySearchDescending = (options) => {
  let aArray      = options.aArray;
  let val         = options.val;
  let sProp       = options && validString(options.sProp) ? options.sProp : 'val'
  ...
  // searchElement 
  let currentIndex = -1;
  let minIndex = aArray.length - 1;
  let maxIndex = 0;

  while (minIndex >= maxIndex) {
    currentIndex = Math.floor( (minIndex + maxIndex) / 2);
    while (aArray[currentIndex] == null && currentIndex > minIndex) {
      // console.log('skipping a null at',currentIndex)
      currentIndex++;
    }
...
module.exports = {
  ...
  binarySearchDescending  : binarySearchDescending,
