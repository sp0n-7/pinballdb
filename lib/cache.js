...
class Cache extends CacheBase {
  ...
  upsert(oDoc) {
    ...
    aPromises.push(this.client.zaddAsync(sortedSet, oDoc.ts, cacheId));
  ...
  batchGetFromCache(options) {
    ...
    //   }
    //   batch.exec( (err,aResponse) => {
    //     // console.log('batchGetFromCache', aResponse);
    //     if (err) {
    //       reject(err);
    //     } 
    //     else {
    ...
    const nextBatch = (iStart,iEnd) => {
      ...
      return new Promise( (resolve,reject) => {
        for (let i = iStart;i < iEnd;i++) {
          ...
          batch.zadd(sortedSet, oDoc.ts, cacheId); // use created time for ordered set
