...
class CacheBase {
  constructor(options) {
    // this.sClass = 'Cache';
    if (typeof options.sCacheUrl !== 'string' || options.sCacheUrl.length === 0 || typeof options.setName !== 'string') {
      throw Error(`CacheBase invalid inputs options ${options}`)
    }
    ...
    const nextBatch = (iStart,iEnd) => {
      ...
      return new Promise( (resolve,reject) => {
        for (let i = iStart;i < iEnd;i++) {
          ...
          if (typeof cityCode === 'string' && cityCode.length > 0 && typeof oDoc.cs === 'number') {
