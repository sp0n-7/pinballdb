...
const removeNulls = (oData) => {
	...
	// omitBy recursive
	return omitByRecursively(oData, _.isNil)
}
...
const getDocId = (cacheId) => {
	...
	if (typeof cacheId === 'string') {
		...
		if (Array.isArray(aResults) && aResults.length >= 2) {
			...
			console.error({ action: sAction + '.err', cacheId: cacheId, setName: 'pb', aResults: aResults })
		}
	}
	return oDocId
}
...
class CacheBase {
	constructor (options) {
		...
		options.setName = u.validString(options.setName) ? options.setName : 'pb'
		options.scoreProperty = u.validString(options.scoreProperty) ? options.scoreProperty : 'cs'
	...
	getDocId (cacheId) {
		const sAction = 'CacheBase.getDocId'
		let oDocId = null
		if (typeof cacheId === 'string') {
			...
			if (Array.isArray(aResults) && aResults.length >= 2) {
				...
				console.error({ action: sAction + '.err', cacheId: cacheId, setName: this.setName, aResults: aResults })
			}
		}
		return oDocId
	}
...
class CacheSubscriber extends CacheBase {
	...
	async subscribe (oProcs) {
		this.oProcs = oProcs
		// for (let i in this.oProcs) {
		//   this.oProcs[i].bind(this)
		// }
		...
		// return a promise
		return Promise.all(aSubscribePromises)
	}
}

const subscribeToCache = (options) => {
	const sub = new CacheSubscriber(options)
	return sub.subscribe(options.oProcs).then(() => {
		return sub
	})
}
...
class Cache extends CacheBase {
	...
	upsert (oDoc) {
		...
		oDoc = removeNulls(oDoc) // never allow null properties to be stored in cache, can crash clients
		...
		return Promise.all(aPromises).then(aRes => {
			...
			console.error('WKM ERROR UPSERTING', err)
			return Promise.resolve()
		})
	}
	...
	upsertFromIncident (options) {
		const id = options.id
		const oIncident = removeNulls(options.val) // never allow null properties to be stored in cache, can crash clients
		...
		return this.upsert(oCacheIncident).catch((err) => {
			console.error('WKM ERROR UPSERTING', err)
			return Promise.resolve()
		})
	}
	...
	async batchGetFromCacheByScore (options) {
		const sAction = Cache.name + '.batchGetFromCacheByScore'
		const cityCode = options.cityCode
		const setKey = this.getSortedSetName(cityCode)

		// zrangebyscore get all the keys
		const aCacheIds = await this.client.zrangebyscoreAsync(setKey, options.scoreStart, '+inf')

		// then batch get from key array
		let oGetOptions = {
			aCacheIds: aCacheIds // from zrangebyscore
		}
		if (options.aProps) {
			oGetOptions.aProps = options.aProps
		}
		return this.batchGetFromCache(oGetOptions)
	}
		...
		const nextBatch = (iStart, iEnd) => {
			...
			return new Promise((resolve, reject) => {
				for (let i = iStart; i < iEnd; i++) {
					let oDoc = removeNulls(aDocArray[i]) // never allow null properties to be stored in cache, can crash clients
			...
			return new Promise((resolve, reject) => {
				...
				for (let i = iStart; i < iEnd; i++) {
					...
					if (oDocId != null && u.validString(oDocId.id)) {
						...
						console.error({ action: `${sAction}.${cityCode}.getDocId.err`, cacheId: cacheId })
					}
				}
				batch.exec((err, aResponse) => {
