...
// got a weird error, callback is not a function, bluebird.promisifyAll(zlib);
const gzip              = bluebird.promisify(zlib.gzip);
const gunzip            = bluebird.promisify(zlib.gunzip);

// const gzip = inData => {
//   return new Promise( (resolve,reject) => {
//     zlib.gzip(inData, (err,outData) => {
//       if (err) {
//         reject(err);
//       }
//       else {
...
// const gunzip = inData => {
//   return new Promise( (resolve,reject) => {
//     console.log('inData',inData.toString())
//     zlib.gunzip(inData, (err,outData) => {
//       if (err) {
//         reject(err);
//       }
//       else {
...
const getRedisClient = (sUrl) => {
  ...
  const client    = redis.createClient(redisUrl.port, redisUrl.hostname, {no_ready_check: true, detect_buffers : true });
...
class Cache extends CacheBase {
  ...
  getWithCacheId(cacheId) {
    return this.client.getAsync(cacheId)
    .then( zData => {
      return gunzip(zData);
    })
    .then( sData => {
  ...
  multiGetWithCacheIds(aKeys) {
    if (Array.isArray(aKeys) && aKeys.length > 0) {
      return this.client.mgetAsync(aKeys).then( azData => {
        let aPromises = [];
        for (let i in azData) {
          aPromises.push(gunzip(azData[i]))
        }
  ...
  getWithDocId(options) {
    ...
    return this.getWithCacheId(cacheId);
  }
  ...
  upsert(oDoc) {
    ...
    return gzip(JSON.stringify(oDoc)).then( zData => {
      let aPromises = [];
      aPromises.push(this.client.setAsync(new Buffer(cacheId), zData));
  ...
  batchGetFromCache(options) {
    ...
    const aProps        = options.aProps; // pluck these properties
    ...
    const getNextBatch = (aArray,iStart,NBatchSize,aFullDocs) => {
      return new Promise( (resolve,reject) => {
        ...
        for (let i = iStart;i < iEnd && i < aArray.length;i++) {
          ...
          batch.get(new Buffer(cacheId));
        }
        batch.exec( (err,azResponse) => {
          let aBatchDocs = [];
          if (Array.isArray(azResponse)) {
            let aPromises = [];
            for (let i = 0; i < azResponse.length;i++) {
              aPromises.push(gunzip(azResponse[i]));
            }
            ...
            .catch( err => {
              console.error({ action: sAction + '.gunzip.err', err:err });
              reject(err);
            });
          }
        });
  ...
  batchUpsertCache(aDocArray) {
    const sAction = Cache.name + '.batchUpsertCache';
    ...
    const nextBatch = (iStart,iEnd) => {
      ...
      return new Promise( (resolve,reject) => {
        ...
        for (let i = iStart;i < iEnd;i++) {
          const sDocString = JSON.stringify(aDocArray[i]);
          aPromises.push(gzip(sDocString));
          aDocStrings.push(sDocString);
        }
        Promise.all(aPromises).then( azDocString => {
          for (let i = iStart;i < iEnd;i++) {
            let j = i - iStart;
            const oDoc          = aDocArray[i];
            const sDocString    = aDocStrings[j];
            const zDocString    = azDocString[j];
            ...
            batch.set(new Buffer(cacheId), azDocString[i-iStart]);
        ...
        .catch( err => {
          console.error({ action: sAction + '.gzip.err', err:err });
        })
