...
class Cache extends CacheBase {
  ...
  upsert(oDoc) {
    ...
    if (!util.exists(oDoc.cs)) {
      return Promise.reject(Error(`${sAction}.invalid.doc.err missing cs property, oDoc ${JSON.stringify(oDoc)}`))
    }
    ...
    const nextBatch = (iStart,iEnd) => {
      ...
      return new Promise( (resolve,reject) => {
        for (let i = iStart;i < iEnd;i++) {
          ...
          if (typeof cityCode === 'string' && cityCode.length > 0 && util.exists(oDoc.cs)) {
